...
// API route to provide safe public EmailJS keys to client if configured on server
...
// API route to verify security gate bypass password
app.post('/api/gate/verify-bypass', (req, res) => {
  const { password } = req.body;
  const expectedPassword = process.env.GATE_BYPASS_PASSWORD || 'fuck@duck';
  ...
  if (password.trim() === expectedPassword.trim()) {
    return res.json({ success: true, message: 'Bypass authorization granted' });
  } else {
    return res.status(401).json({ success: false, error: 'Invalid bypass password. Access denied.' });
  }
});

// API route to provide Cloudflare Turnstile public site key safely to client
app.get('/api/turnstile/config', (req, res) => {
  ...
  res.json({
    siteKey,
    isCustomConfigured: Boolean(customSiteKey),
    hasSecret,
  });
});

// API route for server-side Cloudflare Turnstile token verification
app.post('/api/turnstile/verify', async (req, res) => {
  ...
  // Handle test bypass token for developer and preview testing
  if (token === 'bypass-test-mode' || token.startsWith('test-') || token.startsWith('bypass-')) {
    return res.json({ success: true, challenge_ts: new Date().toISOString(), testMode: true });
  }

  const secretKey =
    process.env.CLOUDFLARE_TURNSTILE_SECRET_KEY ||
    process.env.TURNSTILE_SECRET_KEY ||
    '1x0000000000000000000000000000000AA'; // Fallback to Cloudflare test secret key
  ...
  try {
    ...
    if (verifyData.success) {
      return res.json({ success: true, challenge_ts: verifyData.challenge_ts });
    } else {
    ...
    console.error('Error verifying Turnstile token:', err?.message);
    return res.status(500).json({ success: false, error: 'Failed to contact Cloudflare verification endpoint' });
  }
});

// API route for EmailJS secure transmission dispatch
app.post('/api/contact/email', async (req, res) => {
  const { name, email, subject, message, turnstileToken } = req.body;

  const senderName = (name && String(name).trim()) || (email ? String(email).split('@')[0] : 'Operative');
  if (!email || !message) {
    return res.status(400).json({ error: 'Email and message are required' });
  }

  const gmailRegex = /^[a-zA-Z0-9._%+-]+@(gmail|googlemail)\.com$/i;
  if (!gmailRegex.test(String(email).trim())) {
    return res.status(400).json({ error: 'Only valid Gmail addresses (@gmail.com) are accepted' });
  }

  // Verify Cloudflare Turnstile token if configured or provided
  const turnstileSecret =
    process.env.CLOUDFLARE_TURNSTILE_SECRET_KEY ||
    process.env.TURNSTILE_SECRET_KEY;

  if (turnstileSecret && !turnstileToken) {
    return res.status(403).json({ error: 'Cloudflare security verification required' });
  }

  if (turnstileToken) {
    try {
      const secretToUse = turnstileSecret || '1x0000000000000000000000000000000AA';
      const formData = new URLSearchParams();
      ...
      if (!verifyData.success) {
        return res.status(403).json({
          error: 'Cloudflare security verification challenge failed',
          errorCodes: verifyData['error-codes'],
        });
      }
    } catch (verr: any) {
      console.error('Turnstile verification failed in email dispatch:', verr?.message);
    }
  }
  ...
  if (!serviceId || !templateId || !publicKey) {
    return res.status(503).json({
      error: 'EmailJS secrets not configured on server',
      missing: {
        serviceId: !serviceId,
...
${message}`;
    ...
    if (!response.ok) {
      ...
      return res.status(response.status).json({ error: errorText || 'Failed to dispatch email through EmailJS' });
    }

    return res.json({ success: true, message: 'Dispatched successfully via EmailJS' });
  } catch (error: any) {
    console.error('EmailJS request failed:', error?.message);
    return res.status(500).json({ error: 'Internal server error attempting EmailJS dispatch' });
  }
});
