...
export const CredentialsSection: React.FC = () => {
  ...
  const formatBoldPrefix = (text: string) => {
  ...
  const filteredCertifications = useMemo(() => {
    if (activeTab === 'certifications') return CERTIFICATIONS_DATA;
    if (['cybersecurity', 'cloud', 'automation', 'analytics'].includes(activeTab)) {
      return CERTIFICATIONS_DATA.filter((c) => c.category === activeTab);
    }
    return [];
  }, [activeTab]);
  ...
  return (
    <section id="certificates" className="pt-[15px] px-0 pb-0 border-b-0 bg-transparent relative scroll-mt-28 text-white font-mono">
      <div className="max-w-7xl mx-auto px-[8px] flex flex-col gap-[15px]" style={{ paddingLeft: "8px", paddingRight: "8px" }}>
            ...
            <div className="h-[95px] bg-[#21232b] border-0 p-2 rounded-2xl transition-all flex flex-col justify-between" style={{ paddingLeft: "8px", paddingRight: "8px", paddingTop: "8px", paddingBottom: "8px", height: "95px" }}>
              <div className="h-[30px] flex items-center justify-between">
                <div className="flex items-center gap-2 text-[11px] font-bold text-[#a8c7fa]">
                  <div className="w-[32.9948px] h-[32.9948px] shrink-0 rounded-lg bg-[#a8c7fa] text-[#00325b] flex items-center justify-center text-base font-bold shadow-sm" style={{ width: "32.9948px", height: "32.9948px" }} title="Certifications">
            ...
            <div className="h-[95px] bg-[#21232b] border-0 p-2 rounded-2xl transition-all flex flex-col justify-between" style={{ paddingLeft: "8px", paddingRight: "8px", paddingTop: "8px", paddingBottom: "8px", height: "95px" }}>
              <div className="h-[30px] flex items-center justify-between">
                <div className="flex items-center gap-2 text-[11px] font-bold text-[#fdd663]">
                  <div className="w-[32.9948px] h-[32.9948px] shrink-0 rounded-lg bg-[#fdd663] text-[#3b2f00] flex items-center justify-center text-base font-bold shadow-sm" style={{ width: "32.9948px", height: "32.9948px" }} title="Digital Badges">
            ...
            return (
              <button
                ...
                className={`w-[35px] flex-1 h-[35px] flex items-center justify-center rounded-full transition-colors cursor-pointer text-center border-0 text-[13px] leading-[16px] ${
                  isActive ? activeColorClass : 'text-[#c4c6d0] hover:text-white'
                }`}
                title={tab.label}
          ...
          <div className="flex items-center gap-2">
            <span style={{ fontSize: "12px", lineHeight: "12px" }}>
              ...
              {['cybersecurity', 'cloud', 'automation', 'analytics'].includes(activeTab) ? (
                <>
                  <strong className="text-[#a8c7fa]">{activeCategoryMeta.typeBadge}</strong> <strong className="text-white">CERTS</strong>
                </>
                ...
                <strong className="text-white">{activeCategoryMeta.labelSuffix}</strong>
              )}
            </span>
          </div>
          <div className="flex items-center gap-3">
            <span className="hidden sm:inline-flex items-center gap-1.5 text-[11px] leading-[11px] text-[#8e9199]">
              <i className={`${activeCategoryMeta.icon} ${activeTab === 'badges' ? 'text-[#fdd663]' : activeTab === 'profiles' ? 'text-[#d0bcff]' : 'text-[#a8c7fa]'}`}></i>
              TYPE: <strong className="text-white uppercase">{activeCategoryMeta.typeName}</strong>
            </span>
            {totalPages > 1 && (
              <span className="text-[12px] leading-[12px] text-[#8e9199]" style={{ fontSize: "12px", lineHeight: "12px" }}>
                PAGE{' '}
                <span className={`font-bold ${activeTab === 'badges' ? 'text-[#fdd663]' : activeTab === 'profiles' ? 'text-[#d0bcff]' : 'text-[#a8c7fa]'}`}>
                  {currentPage}
                </span>{' '}
                / {totalPages}
              </span>
                      ...
                      >
                        <div className="flex items-start justify-between gap-3">
                          <div className="space-y-1.5 flex-1 min-w-0">
                            <div>
                              <span className="font-semibold text-[#8e9199] leading-[12px]" style={{ lineHeight: "12px" }}>
                                Issuer:
                              </span>{' '}
                              <span className="font-medium text-white leading-[12px]" style={{ lineHeight: "12px" }}>
                                {cert.issuer}
                              </span>
                            </div>
                            <div>
                              ...
                              <span className="text-[#c2e7ff] capitalize font-medium leading-[12px]" style={{ lineHeight: "12px" }}>
                                {cert.category}
                              </span>
                            </div>
                          </div>
          ...
          <div className="animate-fadeIn">
            <div className="grid grid-cols-1 gap-3.5">
              {paginatedBadges.map((badge) => {
                ...
                const activeStepIdx = isExpanded && selectedPathSteps[badge.id] !== undefined ? selectedPathSteps[badge.id] : null;
                const activeSubBadge = isExpanded && activeStepIdx !== null && badge.learningPath && badge.learningPath[activeStepIdx]
                  ? badge.learningPath[activeStepIdx]
                  : null;

                const displayTitle = activeSubBadge ? activeSubBadge.title : badge.title;
                const displayIssuer = activeSubBadge ? activeSubBadge.issuer : badge.issuer;
                const displayDate = activeSubBadge ? activeSubBadge.issueDate : badge.issueDate;
                const displayCredentialId = activeSubBadge ? activeSubBadge.credentialId : badge.credentialId;
                const displayDescription = activeSubBadge ? activeSubBadge.description : badge.description;
                const displayLink = activeSubBadge ? activeSubBadge.link : badge.link;
                ...
                return (
                  ...
                  >
                    ...
                    >
                      <div className="flex items-center justify-between gap-2 shrink-0 min-h-[33px]">
                        <div className="flex items-center gap-2.5 text-[11px] font-bold text-[#fdd663] min-w-0 flex-1">
                          <div
                            className="w-[33px] h-[33px] shrink-0 rounded-lg bg-[#fdd663] text-[#3b2f00] flex items-center justify-center font-bold shadow-sm"
                            style={{ width: "33px", height: "33px" }}
                            title={displayIssuer}
                          >
                    ...
                    {isExpanded && (
                      <div
                        className="mt-[15px] min-h-[38px] h-auto bg-[#13141a] border border-[#44474f]/30 rounded-xl p-2 text-[12px] leading-[16.5px] text-[#a8aab3] font-sans space-y-2.5 animate-fadeIn"
                        style={{ paddingLeft: "8px", paddingRight: "8px", paddingTop: "8px", paddingBottom: "8px" }}
                      >
                        {badge.learningPath && badge.learningPath.length > 0 && (
                          <div
                            className="grid gap-1 w-full"
                            style={{ gridTemplateColumns: `repeat(${badge.learningPath.length}, minmax(0, 1fr))` }}
                          >
                        ...
                        <div className="flex items-start justify-between gap-3">
                          ...
                          <div className="flex items-center gap-1.5 shrink-0">
                            ...
                            <a
                              ...
                              className="w-[33px] h-[33px] rounded-lg bg-[#fdd663] hover:bg-[#ffe088] text-[#3b2f00] flex items-center justify-center font-bold shadow-sm transition-all shrink-0 cursor-pointer"
                              style={{ width: "33px", height: "33px" }}
                            >
                              <i className="ri-link-unlink-m text-[27px]" style={{ fontSize: "27px" }}></i>
                            </a>
                        ...
                        {displayDescription && (
                          <div className="pt-2 text-[#a8aab3] text-justify text-[12px] leading-[16.5px]">
                            {formatBoldPrefix(displayDescription)}
                          </div>
                        )}
                      </div>
                    )}
        ...
        {activeTab === 'profiles' && (
          <div className="animate-fadeIn">
            {/* 2-column side by side layout on all screens */}
            <div className="grid grid-cols-2 gap-[15px]" style={{ gap: "15px" }}>
              {paginatedProfiles.map((profile) => {
                return (
                  ...
                  >
                    {/* Card Header (Expanded by default, no expand/close button) */}
                    ...
                    {/* Permanently expanded description */}
                    {profile.description && (
                      <div
              ...
              {Array.from({ length: totalPages }, (_, i) => i + 1).map((pageNum) => {
                ...
                return (
                  <button
                    ...
                    className={`w-9 h-9 rounded-[18px] font-mono text-[13px] leading-[16px] flex items-center justify-center transition-all cursor-pointer border-0 ${
                      isCurrent
                        ? activeTab === 'badges'
                          ? 'bg-[#fdd663] text-[#3b2f00] font-bold shadow-sm'
                          : activeTab === 'profiles'
                          ? 'bg-[#d0bcff] text-[#381e72] font-bold shadow-sm'
                          : 'bg-[#a8c7fa] text-[#003258] font-bold shadow-sm'
                        : 'bg-[#21232b] text-[#c4c6d0] hover:bg-[#2c2f3a] hover:text-white'
                    }`}
                  >
                    {pageNum}
                  </button>
