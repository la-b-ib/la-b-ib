...
===================================================================`,
 ...
 return (
 <section className="w-full h-full flex flex-col min-h-0 bg-[#0f0e13] text-white font-mono animate-fadeIn transition-all pb-[calc(94px+env(safe-area-inset-bottom,0px))]">
 <div className="w-full h-full flex flex-col min-h-0 bg-[#0f0e13]">
 {/* Terminal Header Bar */}
 ...
 <span className="truncate max-w-[180px] xs:max-w-[220px] text-[16px]">TERMINAL</span>
 <span className="hidden text-[10px] text-[#c2e7ff] bg-[#004a77]/40 px-2.5 py-0.5 rounded-full border border-[#a8c7fa]/30 shrink-0">
 SEC_OPS SHELL
 </span>
 </span>
 </div>
 ...
 <div onClick={() => inputRef.current?.focus()} className="flex-1 min-h-0 bg-[#09090d] px-[15px] pt-[15px] pb-[15px] font-mono text-[12px] leading-[14px] overflow-y-auto space-y-2 [scrollbar-width:thin] [scrollbar-color:#44474f_#09090d] [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-track]:bg-[#09090d] [&::-webkit-scrollbar-thumb]:bg-[#44474f] [&::-webkit-scrollbar-thumb]:rounded-full select-text">
 {history.map((item) => {
 ...
 <div className="w-full z-45 bg-black pt-[15px] pb-0 px-[15px] h-[60px] flex justify-center items-center shrink-0">
 <form
 onSubmit={handleSubmit}
 className="w-full max-w-md bg-[#21232b] px-[10px] h-[45px] rounded-xl border border-[#44474f]/40 flex items-center gap-2 transition-colors shadow-[0_8px_30px_rgba(0,0,0,0.7)]"
 >
 <span className="text-[#a8c7fa] font-mono font-semibold text-[16px] leading-[16px] flex items-center shrink-0 gap-1.5">
 <span className="text-[#8e9199] hidden">node-{String(cmdHistory.length).padStart(2, '0')}:</span>
 <i className="ri-money-dollar-circle-line text-[20px] text-[#a8c7fa]"></i>
 </span>
 <div className="relative flex-1 flex items-center h-full">
 <input
 ref={inputRef}
 type="text" aria-label="Terminal command input"
 value={inputVal}
 onChange={(e) => {
 ...
 className="w-full bg-transparent border-none text-white font-mono text-[16px] leading-[16px] focus:outline-none placeholder:text-[#8e9199] placeholder:text-[16px] placeholder:leading-[16px] h-[45px] z-10"
 />
 {ghostText && inputVal.trim() && (
 <span className="absolute left-0 pointer-events-none font-mono text-[16px] leading-[16px] text-[#44474f] whitespace-pre min-h-[36px] flex items-center">
 <span className="opacity-0">{inputVal}</span>
 <span>{ghostText}</span>
 <span className="ml-2 text-[9px] leading-[9px] text-[#a8c7fa]/60 bg-[#004a77]/30 px-1 py-0.5 rounded border border-[#a8c7fa]/20 hidden">
 [TAB]
 </span>
 </span>
 )}
