...
===================================================================`,
 ...
 <div className="w-full h-full flex flex-col min-h-0 bg-[#000000]">
        ...
        <div className="px-0 pt-0 pb-0 shrink-0 bg-[#000000]">
          <div className="bg-[#21232b] h-[45px] p-2 rounded-none border-0 flex items-center gap-2 select-none shrink-0" style={{ paddingLeft: "8px", paddingRight: "8px", paddingTop: "8px", paddingBottom: "8px" }}>
            {/* Terminal Icon & Label */}
            <div className="flex items-center space-x-1.5 shrink-0" style={{ paddingLeft: "0px" }}>
              <button
                type="button"
                onClick={() => {
                  showDefaultScreen();
                }}
                className="w-[32px] h-[32px] flex items-center justify-center rounded-[8px] cursor-pointer text-center border-0 text-[13px] leading-[16px] bg-[#a8c7fa] text-[#042e60] font-semibold active:scale-95 shrink-0"
            ...
            >
              <span className="text-[#a8c7fa] font-mono font-semibold text-[13px] leading-[16px] flex items-center shrink-0">
              ...
              <div className="relative flex-1 min-w-0 flex items-center h-full">
                ...
                {ghostText && inputVal.trim() && (
                  <span className="absolute left-0 pointer-events-none font-mono text-[13px] leading-[16px] text-[#44474f] whitespace-pre flex items-center">
        ...
        <div
          ...
          className="flex-1 min-h-0 bg-[#000000] p-2 font-mono text-[13px] leading-[16px] overflow-y-auto space-y-2 [scrollbar-width:thin] [scrollbar-color:#44474f_#000000] [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-track]:bg-[#000000] [&::-webkit-scrollbar-thumb]:bg-[#44474f] [&::-webkit-scrollbar-thumb]:rounded-full select-text"
 ...
 <span className={`text-[#a8c7fa] font-bold text-[12px] leading-[12px]`} style={{ fontSize: "12px", lineHeight: "12px" }}>{item.text}</span>
 </div>
 );
 }
 ...
 {item.text}
          ...
          return (
            <div key={item.id} className={`break-words ${textColorClass} grid grid-cols-[max-content_max-content_auto_1fr] gap-x-2 gap-y-0.5 items-baseline w-full text-[12px]`} style={{ fontSize: "12px" }}>
              {elements.map((el, elIdx) => {
                if (el.type === 'table') {
                  ...
                  return (
                    <div key={elIdx} className="col-span-full my-2.5 sm:my-3 border border-[#2d3139] rounded-lg bg-[#0a0e17]/95 shadow-xl font-mono overflow-hidden w-full">
                      <table className="w-full text-left border-collapse table-fixed text-[13px] leading-[16px]">
                        <thead>
                          <tr className="bg-[#131924] border-b border-[#2d3139] text-[#8e9199] font-bold uppercase tracking-wider select-none text-[13px] leading-[16px]">
                            {el.headers.map((h, hIdx) => {
                              ...
                              return (
                                <th
                                  key={hIdx}
                                  className={`px-2 sm:px-3 py-1.5 sm:py-2 truncate ${colWidth} text-left text-[13px] leading-[16px]`}
                                >
                                  {h}
                                </th>
                              );
                          ...
                          {el.rows.map((row, rIdx) => {
                            if (isTwoCol) {
                              ...
                              return (
                                <tr
                                  key={rIdx}
                                  className="border-b border-[#1b2230]/70 hover:bg-[#151e2d]/80 transition-colors last:border-b-0 text-[13px] leading-[16px]"
                                >
                                  <td className="px-2 sm:px-3 py-1.5 sm:py-2 text-[#a8c7fa] font-bold truncate text-[13px] leading-[16px]">
                                    {port}
                                  </td>
                                  <td className="px-2 sm:px-3 py-1.5 sm:py-2 text-[#c4c6d0] break-words text-[13px] leading-[16px]">
                                    <span className="text-[#d0bcff] font-semibold">{serviceName}</span>
                                    {auditNote && (
                                      <span className="text-[13px] leading-[16px] block sm:inline sm:before:content-['•_'] text-[#8e9199]">
                            ...
                            if (isThreeCol) {
                              ...
                              return (
                                ...
                                >
                                  ...
                                  <td className="px-2 sm:px-3 py-1.5 sm:py-2 text-[13px] leading-[16px]">
                                    {isStateOpen && (
                                      <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[13px] leading-[16px] font-bold text-[#a8e6cf] bg-[#00522b]/50 border border-[#a8e6cf]/40 shadow-sm select-none">
                                        <span className="w-1 h-1 sm:w-1.5 sm:h-1.5 rounded-full bg-[#a8e6cf] animate-pulse shrink-0" />
                                        OPEN
                                      </span>
                                    )}
                                    {isStateFiltered && (
                                      <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[13px] leading-[16px] font-bold text-[#ffb870] bg-[#4a2800]/50 border border-[#ffb870]/40 shadow-sm select-none">
                                        <span className="w-1 h-1 sm:w-1.5 sm:h-1.5 rounded-full bg-[#ffb870] shrink-0" />
                                        FILTERED
                                      </span>
                                    )}
                                    {isStateClosed && (
                                      <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[13px] leading-[16px] font-bold text-[#ffb4ab] bg-[#690005]/50 border border-[#ffb4ab]/40 shadow-sm select-none">
                            ...
                            return (
                              ...
                              >
                                {row.map((cell, cIdx) => (
                                  <td key={cIdx} className="px-2 sm:px-3 py-1.5 sm:py-2 text-[#c4c6d0] break-words text-[13px] leading-[16px]">
                                    {cell}
                                  </td>
                                ))}
                              </tr>
                ...
                if (tagMatch) {
                  ...
                  if (!restAfterTag.includes(':')) {
                    ...
                    return (
                      <div key={elIdx} className="col-span-full flex items-center gap-2 font-bold pt-2 pb-0.5" style={{ lineHeight: "12px" }}>
                        <span className={`text-[12px] leading-[12px] px-1.5 py-0.5 rounded border font-mono ${headerTagColor} shrink-0 select-none`} style={{ fontSize: "12px", lineHeight: "12px" }}>
                          {rawTag}
                        </span>
                  ...
                  return (
                    <React.Fragment key={elIdx}>
                      <div className="flex items-center py-0.5">
                        <span className={`text-[12px] leading-[12px] px-1.5 py-0.2 rounded border font-mono font-bold ${tagBadgeColor} shrink-0 select-none`} style={{ fontSize: "12px", lineHeight: "12px" }}>
                          {rawTag}
                        </span>
                      </div>
                      <div className="py-0.5 text-[12px] leading-[16px] text-[#a8c7fa] font-mono select-text whitespace-nowrap" style={{ fontSize: "12px" }}>
                        {keyPart}
                      </div>
                      <div className="py-0.5 text-[13px] leading-[16px] text-[#8e9199] font-bold select-none text-center">
                        :
                      </div>
                      <div className="py-0.5 text-[12px] leading-[15px] text-white font-medium break-words min-w-0" style={{ fontSize: "12px", lineHeight: "15px" }}>
                        {valPart}
                      </div>
                    </React.Fragment>
                  );
                    ...
                    return (
                      <React.Fragment key={elIdx}>
                        <div className="flex items-center justify-end pr-1 py-0.5">
                          {isIndented ? (
                            <span className="text-[#8e9199]/70 font-mono text-[13px] leading-[16px] select-none">↳</span>
                          ) : (
                            <span className="w-0 block" />
                          )}
                        </div>
                        <div className={`py-0.5 text-[13px] leading-[16px] ${isIndented ? 'text-[#8e9199]' : 'text-[#a8c7fa]'} font-mono select-text whitespace-nowrap`}>
                          {keyCandidate}
                        </div>
                        <div className="py-0.5 text-[13px] leading-[16px] text-[#8e9199] font-bold select-none text-center">
                          :
                        </div>
                        <div className="py-0.5 text-[13px] leading-[16px] text-white/95 font-medium break-words min-w-0">
                          {valPart}
                        </div>
                      </React.Fragment>
                    );
                ...
                return (
                  <div key={elIdx} className="col-span-full whitespace-pre-wrap py-0.5 text-[#c4c6d0] text-[12px] leading-[16px]" style={{ fontSize: "12px" }}>
                    {line}
                  </div>
                );
              })}
