...
export const GlobeGLComponent: React.FC<GlobeGLProps> = ({
 ...
 // Restrict zoom limits strictly between 125% and 200% zoom
 // Base distance is ~320 with globe radius = 100
 // 200% zoom = minDistance (closest zoom) ~160 (altitude ~0.6)
 // 125% zoom = maxDistance (furthest zoom) ~256 (altitude ~1.56)
 controls.minDistance = 160; // 200% zoom maximum
 controls.maxDistance = 256; // 125% zoom minimum
 }
 // Default zoom at 135% -> altitude ~1.44 (base altitude 2.2 / 1.35 ~ 1.63 or distance ~237 => altitude ~1.37)
 globe.pointOfView({ lat: 20, lng: 10, altitude: 1.35 });
 ...
 globe.pointOfView(
 { lat: focusLocation.lat, lng: focusLocation.lng, altitude: 1.8 },
 1200
 );
 }
 }, [focusLocation]);
 ...
 return (
 <div className="relative w-full h-full rounded-none border-0 overflow-hidden select-none bg-[#000000] [&_canvas]:!h-[250px]" style={{ height: "250px" }}>
