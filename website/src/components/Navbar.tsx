...
export const Navbar: React.FC<NavbarProps> = ({
 ...
 return (
 <div ref={navRef} className="w-full py-2.5 relative">
 <div className="max-w-7xl mx-auto px-[15px] flex items-center justify-between gap-4">
 ...
 <i className={mobileMenuOpen ? 'ri-close-circle-line text-lg font-bold' : 'ri-list-indefinite text-lg font-bold'}></i>
      ...
      {mobileMenuOpen && (
        <div className="absolute top-full left-0 right-0 w-full bg-[#000000] backdrop-blur-2xl border-b-0 px-[15px] pt-[15px] pb-[15px] text-xs max-h-[85dvh] overflow-y-auto shadow-2xl rounded-b-3xl mt-0 animate-fadeIn z-50">
          {/* Mobile Search Input - Sticky */}
          <div className="w-full mb-[15px] sticky top-0 z-30 bg-[#000000]">
 <HeaderSearch
 placeholder=""
 iconPosition="right"
 iconName="ri-menu-search-line"
 inputClassName="rounded-[16px] border-0 !border-0 focus:ring-0 focus:border-0 outline-none"
 ...
 {/* M3 Segmented Navigation Pills Grid */}
          <div className="grid grid-cols-2 gap-2 mb-[15px]">
 {navLinks.map((link) => {
 const sectionId = link.href.replace('#', '');
 const isActive = activeSection === sectionId;
 ...
 {/* M3 Utility Controls Bar */}
          <div className="pt-0 border-0 flex items-center justify-between gap-2">
 {/* SFX Toggle */}
 <button
 onClick={() => {
