...
export function App() {
  ...
  React.useEffect(() => {
    ...
    const handleFocusOut = () => {
      // Cleanly reset outer body scroll when keyboard closes
      setTimeout(() => {
        window.scrollTo(0, 0);
  ...
  return (
    <div className="h-[var(--visual-viewport-height,100dvh)] sm:min-h-[100dvh] w-full bg-[#09080c] text-[#e3e2e6] flex flex-col items-center justify-start sm:justify-center font-sans antialiased sm:py-6 sm:px-4 overflow-hidden">
      ...
      <div className="w-full sm:max-w-[430px] h-[var(--visual-viewport-height,100dvh)] sm:h-[880px] sm:max-h-[92dvh] bg-[#0f0e13] border-0 sm:border sm:border-[#49454f]/40 sm:rounded-[40px] shadow-2xl relative flex flex-col justify-start overflow-hidden sm:shadow-[0_0_50px_rgba(0,0,0,0.8)]">
        ...
        <main className={`relative z-10 flex-1 flex flex-col px-0 mt-0 min-h-0 ${terminalOpen || ctfOpen || searchOpen ? "overflow-hidden" : "overflow-y-auto overscroll-contain touch-pan-y"} [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden`}>
          {terminalOpen ? (
            <TerminalModal
              isOpen={terminalOpen}
