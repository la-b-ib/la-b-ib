  ...
  return (
    <section className={`w-full flex-1 flex flex-col bg-[#0f0e13] ${isMaximized ? 'min-h-[calc(100dvh-70px)]' : 'min-h-[calc(100dvh-191px)] lg:min-h-[calc(100dvh-92px)]'} mb-[99px] lg:mb-0 text-white font-mono animate-fadeIn transition-all`}>
      <div className="w-full flex-1 flex flex-col bg-[#0f0e13]">
        {/* Terminal Header Bar */}
        <div className="bg-[#1a1b21] px-[15px] py-2.5 sm:py-3 border-b border-[#44474f]/30 flex items-center justify-between select-none">
        ...
        {showSuggestions && (
          <div className="bg-[#14151a] px-[15px] py-2 sm:py-2.5 border-b border-[#44474f]/30 animate-fadeIn transition-all">
            <div className="grid grid-cols-4 sm:flex sm:flex-wrap items-center gap-1.5 sm:gap-2 w-full">
              {quickPills.map((pill) => (
                <button
                  ...
                  style={{ borderRadius: '12px' }}
                  className="w-full sm:w-auto px-1.5 sm:px-3 py-1.5 bg-[#21232b] text-[#c4c6d0] hover:text-white hover:bg-[#2b2d36] active:bg-[#a8c7fa] active:text-[#0f0e13] font-mono text-[10px] sm:text-xs font-medium transition-all active:scale-95 cursor-pointer text-center truncate flex items-center justify-center border-0 outline-none"
        ...
        <div className={`flex-1 ${isMaximized ? 'min-h-[400px] sm:min-h-[580px]' : 'min-h-[260px] sm:min-h-[440px]'} bg-[#09090d] px-[15px] py-3 sm:py-5 font-mono text-[11px] sm:text-sm overflow-y-auto space-y-2 sm:space-y-2.5 leading-relaxed [scrollbar-width:thin] [scrollbar-color:#44474f_#09090d] [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-track]:bg-[#09090d] [&::-webkit-scrollbar-thumb]:bg-[#44474f] [&::-webkit-scrollbar-thumb]:rounded-full select-text`}>
          {history.map((item) => {
            ...
            const hasSeparator = lines.some((line) => /^\s*={3,}\s*$/.test(line) || /^\s*-{10,}\s*$/.test(line));

            if (!hasSeparator) {
              return (
                <div key={item.id} className={`break-words whitespace-pre-wrap ${textColorClass}`}>
                  {item.text}
                </div>
              );
            }
                ...
                {lines.map((line, idx) => {
                  if (/^\s*={3,}\s*$/.test(line) || /^\s*-{10,}\s*$/.test(line)) {
                    return (
                      <div
                        key={idx}
                        className="w-full h-[4px] bg-[#2596be] my-2 rounded-full shrink-0"
                      />
                    );
                  }
                  return (
        ...
        {/* Command Input Form */}
        <form onSubmit={handleSubmit} className="bg-[#1a1b21] px-[15px] py-2.5 sm:py-4 border-t border-[#44474f]/30 flex items-center gap-2 sm:gap-3">
          <span className="text-[#a8c7fa] font-mono font-semibold text-xs sm:text-sm flex items-center shrink-0">
          ...
          <div className="relative flex-1 flex items-center">
            <input
              ...
              className="w-full bg-transparent border-none text-white font-mono text-xs sm:text-sm focus:outline-none placeholder:text-[#8e9199] placeholder:text-[11px] sm:placeholder:text-xs min-h-[36px] z-10"
            />
            {ghostText && inputVal.trim() && (
              <span className="absolute left-0 pointer-events-none font-mono text-xs sm:text-sm text-[#44474f] whitespace-pre min-h-[36px] flex items-center">
                <span className="opacity-0">{inputVal}</span>
                <span>{ghostText}</span>
                <span className="ml-2 text-[9px] text-[#a8c7fa]/60 bg-[#004a77]/30 px-1 py-0.5 rounded border border-[#a8c7fa]/20 hidden md:inline">
                  [TAB]
                </span>
              </span>
            )}
          ...
          <button
            type="submit"
            style={{ borderRadius: '9999px' }}
            className="w-8 h-8 bg-[#a8c7fa] hover:bg-[#a8c7fa]/90 active:scale-95 text-[#0f0e13] font-mono font-bold border-0 transition-all shrink-0 cursor-pointer shadow-sm flex items-center justify-center rounded-full"
            title="Execute command"
            aria-label="Execute command"
          >
            <i className="ri-connector-line text-lg font-bold"></i>
          </button>
        </form>
      </div>
    </section>
