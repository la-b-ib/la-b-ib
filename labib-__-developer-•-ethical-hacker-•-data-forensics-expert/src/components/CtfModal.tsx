...
export const CtfModal: React.FC<CtfModalProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <section className="w-full flex-1 flex flex-col bg-[#0f0e13] min-h-[calc(100dvh-92px)] text-white font-mono animate-fadeIn">
      ...
      <div className="bg-[#21232b] px-4 h-[45px] flex items-center justify-between select-none">
        <div className="flex items-center space-x-3">
          <span className="text-xs font-mono font-semibold text-white flex items-center space-x-2">
            <button className="w-8 h-8 rounded-lg flex items-center justify-center text-base font-bold transition-colors shrink-0 bg-[#fdd663] text-[#3b2f00] cursor-default">
        ...
        <div className="flex items-center space-x-2">
          <button
            ...
            className="w-9 h-9 rounded-full transition-all flex items-center justify-center cursor-pointer active:scale-95 bg-[#ffb4ab] text-[#690005] shadow-md hover:opacity-90 shrink-0"
            title="Exit CTF section [ESC]"
          >
            <i className="ri-close-circle-line text-lg font-bold leading-none"></i>
          </button>
        </div>
      </div>
      ...
      <div className="flex-1 w-full max-w-4xl mx-auto px-4 sm:px-8 pt-[15px] pb-8 flex flex-col justify-start">
        ...
        <div className="bg-[#21232b] p-[15px] h-[170px] rounded-2xl space-y-4 font-mono text-xs">
          ...
          <div className="h-[45px] bg-[#0f0e13] px-[15px] mb-[15px] rounded-xl border border-[#44474f]/30 text-[#fdd663] text-xs sm:text-sm font-mono select-all flex items-center justify-between gap-2 overflow-hidden">
            ...
            <form onSubmit={handleVerifyFlag} className="space-y-4 font-mono text-xs pt-0">
              <div className="relative">
                <input
                  ...
                  className={`w-full h-[45px] bg-[#0f0e13] rounded-xl px-[15px] pr-12 ${errorMsg ? 'text-transparent' : 'text-white'} font-mono text-[12px] focus:outline-none transition-colors`}
                />
